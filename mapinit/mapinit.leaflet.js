    ...
    this.initMap = async () => {
      ...
      for (let i = 0; i < layers.length ?? 0; i++) {
        ...
        if (lyr.type === 'tileLayer') {
          _mapOptions.layers[i] = await createTileLayer(lyr.url, lyr.options)
        }
      }
...
export async function mapInit (divId, options) {
  for (let i = 0; i < options?.layers?.length ?? 0; i++) {
    const lyr = options.layers[i]
    if (lyr.type === 'tileLayer') {
      options.layers[i] = await createTileLayer(lyr.url, lyr.options)
    }
  }
  /**
   * @type {$L.MapOptions}
  ...
  const map = $L.map(divId, mapOptions)
  return map
}
